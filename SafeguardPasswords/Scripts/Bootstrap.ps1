...
try {
    ...
    $config = @{
        venvPath            = $VENV_DIR
        aliasesPath         = $ALIASES
        connectorProjPath  = $(Join-Path -Path $PROJECT_DIR -ChildPath "SafeguardPasswords.proj")
        connectorMezPath   = $(Join-Path -Path $PROJECT_DIR -ChildPath "Deploy\SafeguardPasswords\Bin\OneIdentitySafeguardForPrivilegedPasswords.mez")
        accountOwnershipReport = $(Join-Path -Path $PROJECT_DIR -ChildPath "ReportTemplates\AccountOwnershipReport.pbit")
        assetOwnershipReport = $(Join-Path -Path $PROJECT_DIR -ChildPath "ReportTemplates\AssetOwnershipReport.pbit")
        entitlementReport = $(Join-Path -Path $PROJECT_DIR -ChildPath "ReportTemplates\EntitlementReport.pbit")
        metricsReport = $(Join-Path -Path $PROJECT_DIR -ChildPath "ReportTemplates\MetricsReport.pbit")
        testProjPath       = $(Join-Path -Path $PROJECT_DIR -ChildPath "Test\SafeguardPasswordsTest.proj")
