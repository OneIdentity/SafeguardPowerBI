...
try {
    ...
    New-Variable -Name VENV_DIR -Value $(Join-Path -Path $(Resolve-Path -Path "$ENVIRONMENT_DIR\..").Path -Childpath ".venv") -Option Constant
    ...
    Get-ChildItem -Path $dir\..\..\ -Filter "Safeguard*" | ForEach-Object -Process {
        ...
        $reportTemplatesPath = $(Join-Path -Path $PROJECT_DIR -ChildPath $pathConfig.ReportTemplatePath)
        $reportTemplatePaths = @{}
        $pathConfig.ReportTemplates.psobject.properties | ForEach-Object { $reportTemplatePaths[$_.Name] = $(Join-Path -Path $reportTemplatesPath -ChildPath $_.Value) }
